...
<!--
  Line 1 above is the Linear link and it must be the first line of the body.
    ...
    (The release PR staging -> main carries neither; it lists ticket ids in plain text.)
  No ticket yet? Create one first. See docs/LINEAR_AND_GITHUB.md.

  The base is staging: gh pr create --base staging. The default branch is main and a PR
  opened without --base targets the wrong branch.
  ...
  Numbers, log lines, ini keys and the game's own strings stay quotable.
-->
...
- [ ] `docs/darkness2/ENGINE_NOTES.md` for any address, offset, layout or falsified mechanism
- [ ] `docs/TRAPS.md` for any new trap or failed plan
