...
| VDXR (launch 4) | `launch-game.ps1 -WaitBanner`, the same build and ini with `XrRuntimeJson` empty: `xr: 32-bit ActiveRuntime: ...virtualdesktop-openxr-32.json`, `xr: instance created on runtime 'VirtualDesktopXR' 1.0.10`; the headset was not connected, so `xr: no headset connected (runtime 'VirtualDesktopXR') - will keep retrying quietly` and the game ran flat at 64 Hz with the method producing nothing (`none/s=64`), which is the fail-soft path: the session comes up when Virtual Desktop connects (5 s retry) with no relaunch. The headset judgement is the user's (VR-243) |
