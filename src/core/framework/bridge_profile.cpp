...
void report() {
    for(int k=0;k<StageCount;++k) for(int e=0;e<3;++e) {
        ...
        D2VR_INFO("perf/bridge: %s eye=%d context=%s calls=%u issued=%u resolved=%u pending=%u latePolls=%u full=%u invalid=%u errors=%u discarded=%u GPUms mean=%.4f p50=%.4f p95=%.4f max=%.4f stored=%u overflow=%u; stage interval, not pair latency or additive GPU busy time",
...
bool make(ID3D11Device* d, Slot& s) {
    ...
    if(FAILED(hr)) { release(s.disjoint); release(s.a); release(s.b);
        D2VR_WARN("perf/bridge: CreateQuery failed 0x%08lx; this stage disabled until device reset",(unsigned long)hr); return false; }
    return true;
}
...
void set_enabled(bool value) {
    if(on.exchange(value)!=value) { ++requestedEpoch; D2VR_INFO("perf/bridge: %s; randomized 1/16 stage opportunities, at most one bracket per Present, no query flush/wait",value?"ON":"off"); }
}
...
int begin(ID3D11Device* d,ID3D11DeviceContext* ctx,Stage stage,int eye) {
    ...
    if(device!=d) { reset(); device=d;device->AddRef();thread=GetCurrentThreadId();
        D2VR_INFO("perf/bridge: device epoch started; query results exclude D3D9 scene, XR wait/compositor and encoding"); }
    ...
    if(epoch!=wanted) { clear_stats();epoch=wanted;logAt=now+3000;
        D2VR_INFO("perf/bridge: context boundary -> %s; partial statistics discarded",play.load()?"gameplay":"menu"); }
