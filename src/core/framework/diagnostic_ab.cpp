#define D2VR_CAT ::d2vr::log::Cat::perf
...
void close() {
    ...
    D2VR_INFO("perf/diag-ab: segment=%d reduced=%d n=%u rejected=%u overflow=%u valid=%d fresh-pair ms mean=%.3f p50=%.3f p95=%.3f p99=%.3f max=%.3f rate=%.2f/s over16.667=%u over33.333=%u; BOTH captured serials renewed, not eye Presents or display FPS",
...
void set_enabled(bool value) {
    ...
    D2VR_INFO("perf/diag-ab: %s; baseline/reduced/baseline, 10s settle then 3x33s (first3s excluded); masks only ZAccount/PairTrace/FrameId/AttachCensus; original INI flags never changed",
...
void tick(bool gameplay) {
    ...
    if(invalid.exchange(false) && segment>=0) {
        D2VR_INFO("perf/diag-ab: ABORTED device/level boundary; comparison invalid");set_enabled(false);return;
    }
    if(!gameplay) {
        waitStart=0;
        if(segment>=0) {D2VR_INFO("perf/diag-ab: ABORTED gameplay loss; comparison invalid");set_enabled(false);}
        return;
    }
    ...
    if(++segment==3) {
        D2VR_INFO("perf/diag-ab: COMPLETE valid=%d; baseline p50=%.3f/%.3f p99=%.3f/%.3f; reduced p50=%.3f p99=%.3f. Compare with BOTH baselines and verify fixed scene before claiming benefit",
    ...
    D2VR_INFO("perf/diag-ab: BEGIN segment=%d reduced=%d duration33s warmup3s",segment+1,segment==1);
}
void submit(bool stereo,uint32_t left,uint32_t right) {
    if(!armed||segment<0||!playing||invalid.load())return;
    double now=d2vr::clock::now_ms();bool fresh=pair.accept(stereo,left,right);
