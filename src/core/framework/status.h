...
// on the `status` command. tools\status-dump.ps1 reads it; the harness asserts
// on it. The fields come from a provider the game layer registers
// (game/darkness2/commands.cpp), so the file and the log draw from the same
// numbers.
#pragma once
#include <stdint.h>
#include <stddef.h>
...
public:
    ...
    void kv_hex(const char* key, unsigned long v);   // "0x%08lx" as a string
