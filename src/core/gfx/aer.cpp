...
//                 script lane (d2vr::camera::set_eye is called from the game
...
//   end_frame     capture the game's frame (d2vr::capture) into THIS eye's
...
//                 d2vr::vr::current_eye_sign / the g_aerEnabled path in
...
public:
    ...
    bool end_frame(const FrameDevices&, FrameOutput&) override {
        D2VR_LOG_ONCE(D2VR_CAT, ::d2vr::log::Level::Warn, "stereo: aer end_frame reached - %s", note());
        return false;
    }
    void on_reset() override {}
    void shutdown() override {}
    void status(d2vr::status::Writer& w) override { w.kv("aer", "design stub"); }
};
