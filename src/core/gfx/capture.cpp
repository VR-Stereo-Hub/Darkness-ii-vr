...
void cost_tick() {
    ...
    if (g_mode == Mode::Off) {
        ...
        D2VR_INFO("capture: OFF by request - 0 grabs by design (%u presents skipped in %.1f s), texture() re-shows "
    ...
    if (g_windowGrabs) {
        ...
        D2VR_INFO("capture: cost/present rtd=%u lock=%u copy=%u upload=%u blit=%u total=%u us (%u grabs in "
        ...
        if (g_mode == Mode::Shared)
            D2VR_INFO("capture: shared delivery %s | blit fence waits %u of %u grabs this window (timeouts %u lifetime) "
...
void probe_shared(IDirect3DDevice9* dev, ID3D11Device* dev11) {
    ...
    if (ex) ex->Release();
    D2VR_INFO("capture/probe: the game's device %s IDirect3DDevice9Ex (%s)",
    ...
    if (FAILED(hr) || !rt) {
        D2VR_INFO("capture/probe: CreateRenderTarget %ux%u fmt=%d with a shared handle -> 0x%08lx%s",
        ...
        D2VR_INFO("capture/probe: shared surface REFUSED - the D3D9 device cannot share; the CPU "
                 "readback stays, [Capture] Mode=deferred is the cheaper path (ROADMAP S1)");
        return;
    }
    D2VR_INFO("capture/probe: CreateRenderTarget with a shared handle -> OK, handle=%p", shared);
    ID3D11Texture2D* tex = nullptr;
    const HRESULT hr2 = shared ? dev11->OpenSharedResource(shared, __uuidof(ID3D11Texture2D), (void**)&tex)
                               : E_HANDLE;
    if (FAILED(hr2) || !tex) {
        D2VR_INFO("capture/probe: OpenSharedResource on the mod's D3D11 device -> 0x%08lx", (unsigned long)hr2);
        D2VR_INFO("capture/probe: shared surface REFUSED - D3D9 created it but D3D11 cannot open it; "
                 "[Capture] Mode=deferred is the cheaper path");
    } else {
        D3D11_TEXTURE2D_DESC td = {};
        tex->GetDesc(&td);
        g_sharedOk = true;
        D2VR_INFO("capture/probe: shared surface AVAILABLE - D3D9 %ux%u opened as D3D11 %ux%u fmt=%d "
                 "(no CPU round trip: [Capture] Mode=shared)",
                 g_w, g_h, td.Width, td.Height, (int)td.Format);
        if (g_modeWant != Mode::Shared)
            D2VR_WARN("capture/probe: the device can share but the capture mode is %s, so the readback (and its "
...
bool ensure_texture(ID3D11Device* dev) {
    ...
    if (FAILED(dev->CreateTexture2D(&td, nullptr, &g_tex)) ||
        ...
        D2VR_ERROR("capture: D3D11 texture %ux%u failed - nothing reaches the headset", g_w, g_h);
        if (g_tex) { g_tex->Release(); g_tex = nullptr; }
        return false;
    }
    g_texW = g_w; g_texH = g_h;
    D2VR_INFO("capture: D3D11 game texture %ux%u (BGRA)", g_w, g_h);
    return true;
}
...
bool ensure_deferred(IDirect3DDevice9* dev) {
    for (int i = 0; i < 2; ++i) {
        ...
        if (FAILED(dev->CreateRenderTarget(g_w, g_h, g_fmt, D3DMULTISAMPLE_NONE, 0, FALSE, &g_rt[i], nullptr)) ||
            ...
            !g_sys[i]) {
            D2VR_ERROR("capture: deferred copy target / readback surface %ux%u failed - back to sync", g_w, g_h);
            release_deferred();
            return false;
        }
        g_rtValid[i] = false;
...
bool ensure_shared_slot(IDirect3DDevice9* dev, ID3D11Device* dev11, int i, D3DFORMAT fmt) {
    ...
    if (FAILED(hr) || !g_sharedRt[i] || !shared) {
        D2VR_ERROR("capture: shared render target %d %ux%u fmt=%d refused (0x%08lx; handle %p) - %s", i, g_w, g_h, (int)fmt,
    ...
    if (FAILED(hr) || !g_sharedTex[i]) {
        D2VR_ERROR("capture: OpenSharedResource on slot %d refused (0x%08lx) - D3D9 created it but D3D11 cannot open "
    ...
    if (FAILED(hr) || !g_sharedSrv[i]) {
        ...
        D2VR_ERROR("capture: the SRV on shared slot %d refused (0x%08lx, DXGI fmt %d)", i, (unsigned long)hr, (int)td.Format);
        return false;
    }
    hr = dev->CreateQuery(D3DQUERYTYPE_EVENT, &g_fence[i]);
    if (FAILED(hr) || !g_fence[i]) {
        D2VR_ERROR("capture: the event query for shared slot %d refused (0x%08lx) - no fence, no shared mode", i, (unsigned long)hr);
        return false;
    }
    D3D11_QUERY_DESC qd = {};
    ...
    if (FAILED(hr) || !g_readQuery[i]) {
        D2VR_ERROR("capture: the D3D11 event query for shared slot %d refused (0x%08lx) - no read fence, no shared mode", i, (unsigned long)hr);
        return false;
    }
...
bool ensure_shared(IDirect3DDevice9* dev, ID3D11Device* dev11) {
    ...
    g_sharedFmt = first;
    if (!ok && first != g_fmt) {
        D2VR_WARN("capture: shared slots in fmt=%d refused - retrying in the backbuffer's own fmt=%d", (int)first, (int)g_fmt);
        release_shared();
        ok = ensure_shared_slot(dev, dev11, 0, g_fmt) && ensure_shared_slot(dev, dev11, 1, g_fmt);
        g_sharedFmt = g_fmt;
    }
    if (!ok) {
        D2VR_ERROR("capture: shared mode refused (the lines above say which step) - back to sync");
        release_shared();
        return false;
    }
    D3D11_TEXTURE2D_DESC td = {};
    ...
    LUID luid = {};
    const bool haveLuid = d2vr::d3d9ex::adapter_luid(&luid);
    D2VR_INFO("capture: shared surfaces %ux%u live (2 slots, D3D9 fmt=%d -> D3D11 fmt=%d, D3D9 adapter LUID %08lx-%08lx%s); "
...
bool read_back_queue(IDirect3DDevice9* dev, IDirect3DSurface9* src, IDirect3DSurface9* dst, uint64_t* rtdUs) {
    const long long t0 = qpc_now();
    d2vr::perf::gpu_mark(d2vr::perf::kGpuRtdA);   // the readback copy's own GPU time (perf)
    const HRESULT hr = dev->GetRenderTargetData(src, dst);
    d2vr::perf::gpu_mark(d2vr::perf::kGpuRtdB);
    if (rtdUs) *rtdUs = qpc_us(t0, qpc_now());
    if (FAILED(hr)) {
        if (!g_warnedRtd) {
            g_warnedRtd = true;
            D2VR_ERROR("capture: GetRenderTargetData failed (0x%08lx) - multisampled backbuffer? "
...
void apply_mode_want(IDirect3DDevice9* dev, ID3D11Device* dev11) {
    ...
    if (g_reinitWant) {
        ...
        release_deferred();
        release_shared();
        D2VR_INFO("capture: %s slots REBUILT by request (reinit #%u: released and re-created at this grab, the mode "
    ...
    if (g_modeWant == Mode::Shared && g_probed && !g_sharedOk) {
        D2VR_WARN("capture: mode shared refused - the probe said the device cannot share; staying on %s",
    ...
    release_deferred();
    release_shared();
    D2VR_INFO("capture: mode %s -> %s%s", kModeNames[(int)g_mode], kModeNames[(int)g_modeWant],
...
bool grab(IDirect3DDevice9* dev, ID3D11Device* dev11, ID3D11DeviceContext* ctx) {
    ...
    if (desc.Format != D3DFMT_X8R8G8B8 && desc.Format != D3DFMT_A8R8G8B8) {
        if (!g_warnedFormat) {
            g_warnedFormat = true;
            D2VR_ERROR("capture: backbuffer format %d is not handled (X8R8G8B8/A8R8G8B8 only) - "
    ...
    if (!g_sysmem || desc.Width != g_w || desc.Height != g_h || desc.Format != g_fmt) {
        ...
        if (FAILED(dev->CreateOffscreenPlainSurface(desc.Width, desc.Height, desc.Format,
            ...
            D2VR_ERROR("capture: system-memory surface %ux%u failed", desc.Width, desc.Height);
            bb->Release();
            return false;
        }
        ...
        if (oldW && oldH && (oldW != g_w || oldH != g_h))
            D2VR_WARN("capture: RESOLUTION CHANGED MID-SESSION %ux%u -> %ux%u - the frame the "
            ...
            D2VR_INFO("capture: %ux%u fmt=%d mode=%s", g_w, g_h, (int)desc.Format, kModeNames[(int)g_mode]);
    }
    ...
    d2vr::frameid::stage_backbuffer(dev, bb, thisSerial, thisTag);
    ...
    if (g_mode == Mode::Sync) {
        ...
        const long long t0 = qpc_now();
        d2vr::perf::gpu_mark(d2vr::perf::kGpuRtdA);   // deferred: the blit + the queued readback
        const HRESULT hr = dev->StretchRect(bb, nullptr, g_rt[cur], nullptr, D3DTEXF_NONE);
        blitUs = qpc_us(t0, qpc_now());
        bb->Release();
        if (FAILED(hr)) {
            D2VR_LOG_ONCE(D2VR_CAT, ::d2vr::log::Level::Error,
        ...
        const long long t0 = qpc_now();
        d2vr::perf::gpu_mark(d2vr::perf::kGpuRtdA);   // shared: the blit alone
...
void set_bbox_ms(uint32_t ms) {
    ...
    D2VR_INFO("capture: content-bbox resample %s, was %s - in shared mode each sample is a full-frame "
...
bool set_mode(const char* name) {
    ...
    if (!_stricmp(name, "sync")) m = Mode::Sync;
        ...
        D2VR_WARN("capture: unknown mode '%s' (sync|deferred|shared|off) - staying on %s", name,
                 kModeNames[(int)g_mode]);
        return false;
    }
    if (m == Mode::Off)
        D2VR_WARN("capture: mode off requested - the headset image FREEZES on the last frame by design (the tick "
                 "budget's A/B: what the tick rate is with no capture at all); 'capture mode sync' restores");
    if (m == Mode::Shared && g_probed && !g_sharedOk) {
        D2VR_WARN("capture: mode shared refused - the probe said this device cannot share (the log's "
                 "capture/probe lines say why); staying on %s", kModeNames[(int)g_mode]);
        return false;
    }
    g_modeWant = m;
    if (m != g_mode)
        D2VR_INFO("capture: mode %s requested (applied at the next grab)", kModeNames[(int)m]);
    return true;
}
...
bool request_reinit() {
    if (g_mode != Mode::Shared && g_mode != Mode::Deferred) {
        D2VR_WARN("capture: reinit refused - mode %s has no slots to rebuild (shared|deferred only)", kModeNames[(int)g_mode]);
        return false;
    }
    g_reinitWant = true;
    D2VR_INFO("capture: reinit requested - the %s slots are released and re-created at the next grab, mode unchanged",
             kModeNames[(int)g_mode]);
    return true;
}
...
void set_shared_wait(bool on) {
    ...
    g_sharedWait = on;
    D2VR_INFO("capture: shared delivery -> %s", on ? "this present after its fence (SharedWait=1: zero latency, the CPU waits for the frame in flight)"
...
void on_reset() {
    ...
    release_shared();
    d2vr::frameid::on_reset();   // its 64x64 D3D9 ring is default pool too
    g_w = g_h = 0;
