...
d2vr::vr::PairProbe g_pairBeat;
uint64_t           g_pairBeatMs = 0;
d2vr::vr::PairProbe g_pairPrev;     // the previous beat's cumulative counters (for the deltas)
...
void register_method(IStereo* m) {
    ...
    D2VR_INFO("stereo: method '%s' registered%s%s", m->name(),
...
bool select(const char* name) {
    if (!name || !name[0]) {
        D2VR_WARN("stereo: select needs a name (mono|aer|reentry); staying on '%s'", active_name());
        return false;
    }
    ...
    if (!found) {
        ...
        D2VR_WARN("stereo: no method named '%s' (registered: %s); staying on '%s'", name,
                 g_methodCount ? list : "none", active_name());
        return false;
    }
    if (!found->implemented()) {
        D2VR_WARN("stereo: '%s' is a design stub, not implemented - staying on '%s'. %s",
                 found->name(), active_name(), found->note());
        return false;
    }
    if (found == g_active) {
        D2VR_DEBUG("stereo: '%s' is already the active method", found->name());
        return true;
    }
    ...
    g_active = found;
    g_last = FrameOutput{};
    D2VR_INFO("stereo: method %s -> %s (live; the next present uses it)",
...
void apply_config_method() {
    ...
    D2VR_INFO("stereo: [Stereo] Method=%s Armed=%d applied after the game side registered -> active '%s'%s",
...
bool choose(const char* name) {
    ...
    if (!g_armed) {
        D2VR_INFO("stereo: '%s' selected while parked - it runs when armed (the F10 tickbox, `stereo arm on`)", g_wanted);
        return true;
    }
    return select(name);
}
...
void set_armed(bool on) {
    ...
    if (!on) {
        ...
        D2VR_INFO("stereo: armed -> off (selected '%s', active '%s') - parked on the mono screen; the selection is "
                 "kept, `stereo arm on` or the F10 tickbox re-arms it", g_wanted, active_name());
    } else {
        const bool ok = select(g_wanted);
        D2VR_INFO("stereo: armed -> ON (selected '%s', active '%s')%s", g_wanted, active_name(),
...
bool end_frame(const FrameDevices& d, FrameOutput& out) {
    ...
    if (g_beatMs == 0) g_beatMs = now;
        ...
        D2VR_INFO("stereo: beat method=%s out/s=%.0f L/s=%.0f R/s=%.0f mono/s=%.0f none/s=%.0f %ux%u"
                 "%s%s%s",
                 ...
                 d2vr::capture::mode() == d2vr::capture::Mode::Off
        ...
        d2vr::vr::pair_probe(&g_pairBeat);
        g_pairBeatMs = now;
        if (wants_projection()) {
            const d2vr::vr::PairProbe& p = g_pairBeat;
            const d2vr::vr::PairProbe& q = g_pairPrev;
            const uint32_t submits = p.stereoSubmits - q.stereoSubmits;
            D2VR_INFO("stereo: eyes ageL=%u ageR=%u presents at the last stereo submit (max this window L=%u R=%u; "
        ...
        // The verdict needs branches, so it cannot live inside the D2VR_INFO
        // arguments: the whole block takes the category gate itself.
        if (::d2vr::log::enabled(D2VR_CAT, ::d2vr::log::Level::Info)) {
            const d2vr::vr::PairProbe& p = g_pairBeat;
            const d2vr::vr::PairProbe& q = g_pairPrev;
            ...
            D2VR_INFO("stereo: rate hmd=%s slots/s=%.1f | presents/s=%.0f submits/s=%.0f (one xrEndFrame per pair) "
...
void set_projection_override(int mode) {
    ...
    g_projOverride = m;
    D2VR_INFO("stereo: projection %s (the frame path arms camera mode on the next present: %s)",
...
void set_hold_untagged(int n) {
    ...
    g_holdUntagged = v;
    D2VR_INFO("stereo: hold %d - up to %d consecutive untagged present(s) are held back (the compositor keeps the "
...
void status(d2vr::status::Writer& w) {
    ...
    w.kv("camMode", d2vr::vr::vr_camera_mode());
    w.kv("cineActive", d2vr::vr::cinematic_active());
...
void log_status() {
    D2VR_INFO("stereo: method=%s (selected %s, armed %d) framesOut=%lu last=%ux%u eyeSign=%d nextEye=%d projection=%s (%s) "
             ...
             projection_override_name(), (int)d2vr::vr::vr_camera_mode(), (int)d2vr::vr::cinematic_active(),
             g_methodCount);
    for (int i = 0; i < g_methodCount; ++i)
        D2VR_INFO("stereo:   %s%s%s", g_methods[i]->name(),
                 g_methods[i] == g_active ? " (active)" : "",
                 g_methods[i]->implemented() ? "" : " (design stub)");
    {   // the pair probe as the beat last drained it
        const d2vr::vr::PairProbe& p = g_pairBeat;
        D2VR_INFO("stereo: pair (as of %lu ms ago): ages L=%u R=%u presents (healthy 1/0) | stereoSubmits=%lu pairs=%lu "
        ...
        if (p.phaseAvailable)
            D2VR_INFO("stereo: pair phase %+.1f ms mean (last %+.1f, max %+.1f) over %lu pairs, missed slot %lu (%.0f%%) - "
            ...
            D2VR_INFO("stereo: pair phase n/a (the runtime offers no XR_KHR_win32_convert_performance_counter_time)");
        ...
        const int64_t periodNs = d2vr::vr::display_period_ns();
        if (periodNs > 0)
            D2VR_INFO("stereo: rate hmd=%.2f ms (%.1f Hz) slots/s=%.1f | submits=%lu since start, endFrame mean=%.2f ms "
            ...
            D2VR_INFO("stereo: rate hmd=UNKNOWN - this runtime leaves xrWaitFrame's predictedDisplayPeriod at 0, so "
