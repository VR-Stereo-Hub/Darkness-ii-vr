// core/util/crash.cpp - see crash.h. Shape adopted from the Dishonored VR mod.
#define D2VR_CAT ::d2vr::log::Cat::crash
...
// A MEASUREMENT CARRIES THE IDENTITY OF WHAT IT MEASURED. The crash file is
// opened FILE_APPEND_DATA / OPEN_ALWAYS, so it accumulates across runs; every
// run that writes to it first writes one header naming the run.
char g_ctx[128] = "no context yet";
...
void emit(const char* fmt, ...)
{
    ...
    if (g_crashFile == INVALID_HANDLE_VALUE) {
        char path[MAX_PATH];
        d2vr::paths::in_game_dir(path, "darkness2_vr_crash.txt");
        ...
        if (g_crashFile != INVALID_HANDLE_VALUE && !g_headerDone) {
            ...
            int hn = snprintf(hd, sizeof(hd) - 2,
                ...
                "darkness2vr %s build %s config %s  pid=%lu  %s =====",
                st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond,
                D2VR_VERSION, D2VR_BUILD_ID, D2VR_BUILD_CONFIG, (unsigned long)GetCurrentProcessId(), g_ctx);
            if (hn > 0) {
                hd[hn] = '\r'; hd[hn + 1] = '\n';
                DWORD hw = 0;
    ...
    d2vr::log::write(d2vr::log::Cat::crash, d2vr::log::Level::Error, "%s", g_line);
}
...
LONG WINAPI fingerprint(EXCEPTION_POINTERS* ep)
{
    ...
    if (code == EXCEPTION_ACCESS_VIOLATION && ep->ExceptionRecord->NumberParameters >= 2) {
        // The operation is three-valued: 0 read, 1 write, 8 execute (DEP). An
        // EXECUTE fault whose address IS the instruction pointer means control
        // was transferred into freed memory: a call through a dead pointer, not
        // a stray store.
        ...
        selfTest = ((uintptr_t)at == kSelfTestAddress);
        emit("  access violation %s %p%s", what, at,
             selfTest ? "  <== the `crash test` sentinel: a deliberate fault, dump follows"
    ...
    if (ep->ContextRecord) {
        ...
        for (int i = 0; i < 24; i++) {
            if (!d2vr::mem::range_readable(sp + i, 4)) break;
            uint32_t v = sp[i];
        ...
        for (int i = 0; i < 512 && shown < 10; i++) {
            if (!d2vr::mem::range_readable(sp + i, 4)) break;
            uint32_t v = sp[i];
            // DarknessII.exe: base 0x400000, .text 0x401000-0xEEA730 (ENGINE_NOTES s1)
            if (v >= 0x401000 && v < 0xEEB000) { emit("  stack[%d] = 0x%08lx", i, (unsigned long)v); shown++; }
        }
    }
    // THE WILD-EIP GATE. A VEH sees FIRST-CHANCE exceptions, including ones the
    // engine raises and handles on purpose; dumping on those would cost a
    // multi-hundred-MB write mid-gameplay for a fault that was never fatal.
    // base == 0 means the instruction pointer is in NO loaded image: nothing
    // recovers from that, so it is fatal by construction and gets the dump. The
    // self-test sentinel is the other path, so the dump machinery can be proven
...
// dbghelp is resolved ONCE, at install time. A VEH must never call LoadLibrary:
// it takes the loader lock, and a fault raised while that lock is held would
// deadlock instead of producing the dump we came for.
...
// path that decided this fault was fatal.
bool write_dump(EXCEPTION_POINTERS* ep, const char* why)
{
    ...
    SYSTEMTIME st; GetLocalTime(&st);
    char path[MAX_PATH];
    snprintf(path, sizeof(path), "%s\\d2vr_%04u%02u%02u_%02u%02u%02u.dmp", d2vr::paths::dumps_dir(),
             st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
    ...
    if (f == INVALID_HANDLE_VALUE) {
        emit("minidump FAILED to create %s (err %lu) - is %s writable?",
             path, (unsigned long)GetLastError(), d2vr::paths::dumps_dir());
        return false;
    }
    MINIDUMP_EXCEPTION_INFORMATION mei = { GetCurrentThreadId(), ep, FALSE };
    const MINIDUMP_TYPE flags =
        (MINIDUMP_TYPE)(MiniDumpWithIndirectlyReferencedMemory | MiniDumpWithDataSegs | MiniDumpWithFullMemoryInfo);
    BOOL ok = g_miniDump(GetCurrentProcess(), GetCurrentProcessId(), f, flags,
                         ep ? &mei : nullptr, nullptr, nullptr);
    const DWORD dumpError = ok ? ERROR_SUCCESS : GetLastError();
    CloseHandle(f);
    emit("minidump %s (%s, err %lu): %s", ok ? "written" : "FAILED", why, (unsigned long)dumpError, path);
    return ok != FALSE;
}
...
void install()
{
    ...
    g_installed = true;
    {   // resolve dbghelp here, never inside a handler (loader lock).
    ...
    D2VR_INFO("crash handler installed (fingerprint VEH%s + unhandled filter; dumps in %s; previous filter %p)",
              g_miniDump ? " with minidump" : " WITHOUT minidump - no dbghelp.dll",
              d2vr::paths::dumps_dir(), (void*)g_previous);
}
...
void rearm()
{
    ...
    if (cur != g_ours) {
        ...
        D2VR_LOG_ONCE(D2VR_CAT, d2vr::log::Level::Info, "crash filter re-armed (something displaced it: %p)", (void*)cur);
    }
}
...
void note_teardown(const char* why)
{
    if (InterlockedExchange(&g_teardown, 1) == 0)
        D2VR_INFO("teardown noted (%s): later faults get one line and no dump", why);
}
...
void self_test()
{
    D2VR_WARN("crash test: faulting on purpose at 0x%08lx (the crash file gets the run header, dumps\\ gets a .dmp)",
              (unsigned long)kSelfTestAddress);
    d2vr::log::flush();
    *(volatile int*)kSelfTestAddress = 0xD2;
}
