// core/util/log.cpp - see log.h. Shape adopted from the Dishonored VR mod.
...
const char* const kCatNames[(int)Cat::COUNT] = {
    ...
    "capture", "game", "xr", "sim", "openxr", "xrinput", "perf", "stereo", "lua", "overlay", "camera",
};
...
DWORD  g_flushTick = 0;
const DWORD kFlushMs = 200;   // an fflush per line was a measured microstutter source on Dishonored
...
void init(const char* dir, const char* base)
{
    ...
    if (!g_file) {
        ...
        DWORD rotateError = 0;
        // Ten sessions total. .prev.log is the immediate predecessor (what the
        // tooling reads first); .prev2.log through .prev9.log are older runs.
        ...
        if (g_file && rotateError)
            fprintf(g_file, "log: history rotation failed (%lu); appending to preserve previous evidence.\n", rotateError);
