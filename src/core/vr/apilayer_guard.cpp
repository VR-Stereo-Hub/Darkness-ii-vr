// core/vr/apilayer_guard.cpp - adopted verbatim from the Dishonored VR mod's unity
// fragment. This translation unit adds the includes, the two file statics the
// unity build used to supply, and the d2vr::vr::disable_bad_api_layers() entry
// at the end; the body between is unchanged.
#include <windows.h>
#include <stdio.h>
#include <string.h>
...
static char g_dir[MAX_PATH] = "";   // the game directory (where darkness2_vr.ini lives)
static bool g_algGuard = true;      // [VR] DisableBadApiLayers, read by the guard itself
...
{
    if (disabled) {
        D2VR_LOG(d2vr::log::Cat::openxr, d2vr::log::Level::Info,
    ...
    if (!AlgReadFile(manifest, json, sizeof(json))) {
        D2VR_LOG(d2vr::log::Cat::openxr, d2vr::log::Level::Warn,
    ...
    (*bad)++;
    if (!dis[0]) {
        D2VR_LOG(d2vr::log::Cat::openxr, d2vr::log::Level::Error,
...
static void ApiLayerGuard()
{
    ...
    {
        char ini[MAX_PATH];
        _snprintf(ini, MAX_PATH, "%s\\darkness2_vr.ini", g_dir);
...
{
    strncpy_s(g_dir, sizeof(g_dir), d2vr::paths::game_dir(), _TRUNCATE);
    ApiLayerGuard();
}
} // namespace d2vr::vr
