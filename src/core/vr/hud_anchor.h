...
inline void camera_panel_position(const float grip[3],const float camera[4],float distance,float out[3]) {
    ...
    d2vr::xrmath::quat_rotate(camera[0],camera[1],camera[2],camera[3],unit,forward);
...
inline void follow_grip_orientation(const float grip[4], int hand, float userTiltDeg, float out[4]) {
    ...
    float base[4];
    d2vr::xrmath::quat_mul(grip, tilt, base);
    float nod[4];
    d2vr::xrmath::quat_axis_angle(1.0f, 0.0f, 0.0f, userTiltDeg * 3.14159265f / 180.0f, nod);
    d2vr::xrmath::quat_mul(base, nod, out);
}
...
inline void wrist_position(const float grip[3], const float gripQ[4], const float offset[3],
    ...
    float r[3];
    d2vr::xrmath::quat_rotate(gripQ[0], gripQ[1], gripQ[2], gripQ[3], offset, r);
    out[0] = grip[0] + r[0];
...
inline bool reading_grip_reference(const float grip[4],float placement[4],float orientation[4]) {
    ...
    d2vr::xrmath::quat_mul(q,positionBasis,placement);
    d2vr::xrmath::quat_mul(q,pageBasis,orientation);
    return true;
}
...
inline void reading_tilt(const float attached[4],float degrees,float out[4]) {
    ...
    d2vr::xrmath::quat_mul(attached,pitch,out);
}
...
struct GripPanel {
    ...
    bool orient(const float grip[4],const float initial[4],float out[4]) {
        ...
        if(!valid){const float inv[4]={-q[0],-q[1],-q[2],q[3]};d2vr::xrmath::quat_mul(inv,initial,relative);valid=true;}
        d2vr::xrmath::quat_mul(q,relative,out);return true;
    }
};
...
struct OpeningOrientation {
    ...
    bool capture_upright(const float* camera) {
        ...
        d2vr::xrmath::quat_rotate(camera[0],camera[1],camera[2],camera[3],forward,f);
