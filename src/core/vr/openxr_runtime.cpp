...
#define XRLOG(...) D2VR_LOG(::d2vr::log::Cat::openxr, ::d2vr::log::Level::Info, __VA_ARGS__)
...
// %LOCALAPPDATA%\Darkness2VR as a wide path (the mod's paths module is narrow).
bool data_dir_w(wchar_t* out /*MAX_PATH*/) {
    const char* d = d2vr::paths::data_dir();
...
// d2vr::hud::texture() path, byte-identical for games that never set it.
...
d2vr::mono::Anchor g_hudWorldAnchor;      // the world-locked window's park
...
XrPosef parallel_eye_tag(const XrPosef& l, const XrPosef& r, int eye, float ipdMm) {
    ...
    float off[3];
    d2vr::xrmath::quat_rotate(q[0], q[1], q[2], q[3], axis, off);
    XrPosef out{};
    ...
    do {
        ...
        if (SuspendThread(th) != static_cast<DWORD>(-1)) {
            ...
            if (GetThreadContext(th, &ctx)) {
                ...
                d2vr::crash::probe_begin();   // the __try is only a backstop now
...
// runtime; the release zip carries d2vr_steamvr32.dll (an OpenXR-on-OpenVR
...
//
// Mode override: [VR] Runtime=auto|native|steamvr in darkness2_vr.ini
// (set_runtime_mode, called by the config before init_instance).

// Root data dir (%LOCALAPPDATA%\Darkness2VR), NOT the per-game subdir - the
...
bool xr_root_dir(wchar_t* out /*MAX_PATH*/) {
    ...
    if (FAILED(SHGetFolderPathW(nullptr, CSIDL_LOCAL_APPDATA, nullptr, 0, local)))
        return false;
    swprintf_s(out, MAX_PATH, L"%s\\Darkness2VR", local);
    return true;
}
...
bool shim_dll_path(wchar_t* out /*MAX_PATH*/) {
    ...
    *slash = 0;
    swprintf_s(out, MAX_PATH, L"%s\\d2vr_steamvr32.dll", path);
...
bool shim_write_manifest(const wchar_t* shimDll, wchar_t* manifestOut /*MAX_PATH*/) {
    ...
    swprintf_s(manifestOut, MAX_PATH, L"%s\\d2vr_steamvr32.json", dir);
    ...
    fprintf(f,
            ...
            "    \"name\": \"Darkness2VR SteamVR shim (OpenVR)\",\n"
            "    \"library_path\": \"%s\"\n  }\n}\n",
            escaped);
    fclose(f);
    return true;
...
XrResult try_create_instance(const char* label, bool quietExplainer) {
    ...
    strcpy_s(ici.applicationInfo.applicationName, "darkness2-vr");
    ici.applicationInfo.applicationVersion = 1;
    strcpy_s(ici.applicationInfo.engineName, "darkness2-vr");
    ...
    if (XR_FAILED(r)) {
        ...
        if (r == XR_ERROR_RUNTIME_UNAVAILABLE && !quietExplainer) {
            XRLOG("xr: No native runtime reachable from this 32-bit game. SteamVR 2.17+, "
                  ...
                  "The optional SteamVR shim needs d2vr_steamvr32.dll and openvr_api.dll "
...
void init_instance() {
    ...
    if (g_runtimeJson[0] && GetFileAttributesA(g_runtimeJson) == INVALID_FILE_ATTRIBUTES) {
        XRLOG("xr: WARNING [VR] XrRuntimeJson names a manifest this process cannot read (%s, err %lu) - "
              "IGNORED; the registered runtime is used instead. Clear the key in darkness2_vr.ini.",
    ...
    if (XR_FAILED(r) && mayFallBack) {
        ...
        XRLOG("xr: runtime mode 'steamvr' but d2vr_steamvr32.dll is not beside "
...
void on_present_begin() {
    ...
    g_locateMs.store(d2vr::clock::now_ms(), std::memory_order_relaxed);
...
uint32_t build_laser_from(const LaserSnapshot& ls, XrCompositionLayerQuad* quads,
    ...
    if (ls.muzzle) {
        ...
        d2vr::xrmath::xr_local_trim_quat(ls.modelPitch * kDegToRad,
                                        ls.modelYaw * kDegToRad,
                                        ls.modelRoll * kDegToRad, trim);
        d2vr::xrmath::quat_mul(quat, trim, q2);
        d2vr::xrmath::quat_rotate(q2[0], q2[1], q2[2], q2[3], d0, d);
    } else {
        d2vr::xrmath::xr_local_trim_quat(ls.pitchTrim * kDegToRad,
                                        ls.yawTrim * kDegToRad, 0.0f, trim);
        d2vr::xrmath::quat_mul(quat, trim, q2);
        d2vr::xrmath::quat_rotate(q2[0], q2[1], q2[2], q2[3], fwd, d);
    }
...
bool aim_point_occluded(const XrCompositionLayerProjection* proj, const float p[3], float sizeDeg,
    ...
    for (uint32_t e = 0; e < proj->viewCount && e < 2; ++e) {
        ...
        float l[3];
        d2vr::xrmath::quat_rotate(-v.pose.orientation.x, -v.pose.orientation.y, -v.pose.orientation.z,
...
AimVisualResult build_aim_visual(XrCompositionLayerQuad* quads,
    ...
    D2VR_LOG_EVERY_MS(::d2vr::log::Cat::present, ::d2vr::log::Level::Info, 5000,
...
uint32_t build_control_dots(XrCompositionLayerQuad* quads,
    ...
    float fwd[3];
    d2vr::xrmath::quat_rotate(mid.orientation.x, mid.orientation.y, mid.orientation.z,
...
void log_layer_alignment(const XrCompositionLayerProjectionView* pv, bool projectionLayer,
    ...
    if (!projectionLayer) {
        D2VR_LOG_EVERY_MS(::d2vr::log::Cat::openxr, ::d2vr::log::Level::Info, 5000,
    ...
    D2VR_LOG_EVERY_MS(::d2vr::log::Cat::openxr, ::d2vr::log::Level::Info, 1000,
...
float xr_quat_yaw_deg(float qx, float qy, float qz, float qw) {
    ...
    d2vr::xrmath::quat_rotate(qx, qy, qz, qw, fwd, f);
...
void on_present_end(ID3D11Texture2D* frame) {
    ...
    if (g_cineEnabled.load(std::memory_order_relaxed) && projectionMode) {
        ...
        bool fovMm = d2vr::hud::fov_mismatch();
        bool screenOnly = d2vr::hud::screen_only(); // hack/loading/FMV screens
        ...
        if (active) {
            ...
            if (d2vr::hud::fov_watch(&t, nullptr, &age, 500) && t > 0.05f) {
    ...
    d2vr::hud::set_gate(srFrame);
    d2vr::hud::set_projection_mode(projectionMode);   // 41.x (Dishonored, VR-117): the mode, for the HUD redirect's gate
        ...
        if (backbuffer) {
            ...
            if (XR_SUCCEEDED(acqRes)) {
                ...
                if (imageReady) {
                    ...
                    {
                        d2vr::bridge_profile::Scope sample(g_device,g_context,d2vr::bridge_profile::EyeCopy,
                    ...
                    d2vr::frameid::stage_swapchain(g_device, g_context, g_images[target][index].texture, target, index);
                ...
                if (srFrame || (aerActive && target == g_currentEye && imageSign == currentEyeSign))
                    g_eyeContentSerial[srFrame ? srEye : g_currentEye] =
                        imageReady && XR_SUCCEEDED(contentRelease) ? d2vr::capture::delivered_serial() : 0;
                ...
                if (srFrame) {
                    ...
                    if (g_imageOrientation.load(std::memory_order_relaxed)) {
                        d2vr::pose::Record imageRec={};float q[4]={};
                        const uint32_t recId=d2vr::capture::delivered_rec();
                        const int eye=srEye==0?-1:1;
                        const bool copied=d2vr::pose::copy(recId,&imageRec);
                        const bool applied=d2vr::pose::image_orientation(copied?&imageRec:nullptr,eye,q);
                    ...
                    if (stereo) {
                        ...
                        if (!d2vr::pose::copy(recId, &rec)) {
                            ...
                            const bool haveRender =
                                d2vr::pose::render_yaw_deg(&renderYaw, &renderSerial);
                            if (!haveRender) {
                                ++jNoRender;
                            } else {
                                ...
                                d2vr::pose::check_controls(rec, renderYaw);
                                ...
                                if (now2 - lastJoinMs >= 1000) {
                                    lastJoinMs = now2;
                                    const d2vr::pose::Stats ps = d2vr::pose::stats();
    ...
    if (layerCount == 0 && g_frameState.shouldRender) {
        ...
        D2VR_LOG_EVERY_MS(::d2vr::log::Cat::openxr, ::d2vr::log::Level::Warn, 1000,
    ...
    if (layerCount && projectionMode && g_viewSpace != XR_NULL_HANDLE) {
        ...
        if (qp) {
            ...
            ID3D11Texture2D* t = prov ? prov(g_context) : d2vr::hud::texture(g_context); // alpha-repaired
        ...
        float headFwd[3];
        {
            ...
            d2vr::xrmath::quat_rotate(q.x, q.y, q.z, q.w, fwd, headFwd);
        }
        ...
        for (int i = 0; i < nDesc; ++i) {
            ...
            const d2vr::hudanchor::Crop crop =
                d2vr::hudanchor::crop_rect(hd.Width, hd.Height, d.subrect, d.width, d.height);
            ...
            if (d.anchor == HudAnchor::WindowWorld) {
                ...
                if (g_hudWorldAnchor.valid) {
                    ...
                    float r[3];
                    d2vr::xrmath::quat_rotate(q[0], q[1], q[2], q[3], off, r);
                    const float rx[3] = {1, 0, 0}, uy[3] = {0, 1, 0};
                    d2vr::xrmath::quat_rotate(q[0], q[1], q[2], q[3], rx, right);
                    d2vr::xrmath::quat_rotate(q[0], q[1], q[2], q[3], uy, up);
                    space = g_space;
                ...
                } else d2vr::hudanchor::wrist_position(gp, gq, d.base, d.lift, pos);
                const float toHead[3] = {head[0] - pos[0], head[1] - pos[1], head[2] - pos[2]};
                if (d2vr::hudanchor::too_near(toHead)) { g_hudStatNear.fetch_add(1, std::memory_order_relaxed); continue; }
                if (d2vr::hudanchor::behind_face(toHead, headFwd)) { g_hudStatBehind.fetch_add(1, std::memory_order_relaxed); continue; }
                float oq[4];
                if (d.orient == HudOrient::OpeningPlane) {
                    ...
                    d2vr::hudanchor::follow_grip_orientation(gq, d.hand ? 1 : 0, d.tiltDeg, oq);
                } else {
                    if (d2vr::hudanchor::billboard_degenerate(toHead)) { g_hudStatDegenerate.fetch_add(1, std::memory_order_relaxed); continue; }
                ...
                const float rx[3] = {1, 0, 0}, uy[3] = {0, 1, 0};
                d2vr::xrmath::quat_rotate(oq[0], oq[1], oq[2], oq[3], rx, right);
                d2vr::xrmath::quat_rotate(oq[0], oq[1], oq[2], oq[3], uy, up);
                space = g_space;
    ...
    if(layerCount && layers[0]->type==XR_TYPE_COMPOSITION_LAYER_QUAD) {
        ...
        if(g_monoAnchored.load()) {
            ...
            const bool anchor=context<d2vr::mono::Count && (g_monoMask.load()&(1u<<context));
            if(anchor && !g_monoAnchor.valid) {
                HeadPose h;
                if(peek_head_pose(h) && g_monoAnchor.seed({h.px,h.py,h.pz,h.qx,h.qy,h.qz,h.qw},g_screenDistM.load()))
                    XRLOG("xr: mono anchor seeded context=%s local=(%.3f %.3f %.3f)",d2vr::mono::names[context],
    ...
    d2vr::perf::desktop_ab_submit(measuredStereo, g_eyeContentSerial[0], g_eyeContentSerial[1]);
    d2vr::diag_ab::submit(measuredStereo,g_eyeContentSerial[0],g_eyeContentSerial[1]);
...
void draw_debug_ui() {
    ...
    // hover description (d2vr::ovl::tip). Removed from this game's panel, because they do
    ...
    if (ov::section("VR pacing", ov::Debug, "How the game is paced when the headset is not focused (a system menu, the headset off).")) {
        ...
        if (d2vr::ovl::checkbox("Do not let an unfocused headset pace the game", &detach))
        ...
        if (d2vr::ovl::slider_int("Keepalive every (ms)", &ka, 250, 5000))
        ...
        if (d2vr::ovl::checkbox("Feed the compositor while parked", &feed))
            set_pace_feed(feed);
        ...
        if (ImGui::TreeNode("Present-path phases (last / max us)")) {
            ...
            for (int i = 0; i < kPhCount; ++i)
                ImGui::Text("%-13s %6u / %6u", kPhaseNames[i],
                            ...
                            g_phaseMaxUs[i].load(std::memory_order_relaxed));
            if (d2vr::ovl::button("Reset maxima"))
                for (int i = 0; i < kPhCount; ++i)
    ...
    if (d2vr::ovl::checkbox("VR enabled (paces game to headset)", &enabled))
        g_enabled.store(enabled, std::memory_order_relaxed);
    ov::tip("Off stops sending frames to the headset. Leave on.");

    bool camMode = g_cameraMode.load(std::memory_order_relaxed);
    if (d2vr::ovl::checkbox("VR camera mode (6DOF head drive)", &camMode))
    ...
    if (camMode) {
        ...
        if (d2vr::ovl::checkbox("SR pair pacing (one waitFrame per eye pair)", &pair))
        ...
        if (pair) {
            ...
            if (d2vr::ovl::checkbox("Sync pair rate to headset refresh (judder A/B)", &sync))
            ...
            if (d2vr::ovl::checkbox("Strict pairs (mono for a frame when an eye is stale)", &strict))
        ...
        if (d2vr::ovl::checkbox("Cinematic auto-detect (cutscenes/screens)", &cine))
        ...
        if (cine) {
            ...
            if (d2vr::ovl::checkbox("Cinematics as stereo projection (off = big screen)", &stereoC))
        ...
        if (d2vr::ovl::checkbox("Manual claimed FOV (distortion calibration)", &manualFov)) {
        ...
        if (manualFov) {
            ...
            if (d2vr::ovl::slider_float("Claimed hfov (deg) - stop the swim", &v, 40.0f, 160.0f))
    ...
    if (!camMode) {
        ...
        if(d2vr::ovl::checkbox("Anchor mono screens",&anchored)) set_mono_anchor(anchored,g_monoMask.load());
        ov::tip("The flat screen stays fixed in the world instead of following your head.");
        if(d2vr::ovl::button("Recenter mono screen")) recenter_mono_anchor();
        ov::tip("Puts the flat screen in front of where you look now.");
        if(anchored) {
            uint32_t mask=g_monoMask.load();
            for(unsigned i=0;i<d2vr::mono::Count;++i) {
                bool on=(mask&(1u<<i))!=0;
                if(d2vr::ovl::checkbox(d2vr::mono::names[i],&on)) {
        ...
        if (d2vr::ovl::slider_float("Screen distance (m)", &dist, 0.5f, 5.0f))
            g_screenDistM.store(dist, std::memory_order_relaxed);
        float width = g_screenWidthM.load(std::memory_order_relaxed);
        if (d2vr::ovl::slider_float("Screen width (m)", &width, 0.5f, 6.0f))
...
void handle_cine_command(const char* args) {
    if (strncmp(args, "drive off", 9) == 0) {
        ...
        bool haveFov = d2vr::hud::fov_watch(&t, &tv, &fovAge, 0); // print stale too
        unsigned barsSkipped = 0, barIntervals = 0, barVerts = 0;
        d2vr::hud::get_bar_stats(&barsSkipped, &barIntervals, &barVerts);
        unsigned lbTop = 0, lbBot = 0;
        bool lbPix = d2vr::hud::letterbox(&lbTop, &lbBot);
        bool barDraw = d2vr::hud::bar_draw_active();
        ...
        XRLOG("xr: cine bars=%s (looking for %u verts) | barDraw=%d (skipped %u, "
                ...
                d2vr::hud::bars_hidden() ? "HIDDEN" : "shown", d2vr::hud::bar_verts(),
                barDraw ? 1 : 0, barsSkipped, barIntervals, barVerts, lbPix ? 1 : 0, lbTop,
                lbBot,
                barDraw == lbPix ? "AGREE"
                : d2vr::hud::bars_hidden()
        ...
        d2vr::hud::RouteStats rs{};
        d2vr::hud::get_route_stats(&rs);
        unsigned strandedTotal = 0;
        for (int i = 0; i < d2vr::hud::kRoutePassCount; ++i) strandedTotal += rs.stranded[i];
        XRLOG("xr: cine effects=%s bound=%u verts (inFrame %u, over-bound %u) | "
                ...
                d2vr::hud::effects_in_frame() ? "IN-FRAME" : "panel",
                d2vr::hud::effect_max_verts(), rs.effectsInFrame, rs.effectsRejected,
                rs.stranded[d2vr::hud::kRouteEffect], strandedTotal, rs.restored,
                d2vr::hud::restore_rt() ? "ON" : "off", rs.postFx,
                rs.postFxRejected, rs.squareTarget ? 1 : 0,
                d2vr::hud::postfx_rt_only()
                    ? (d2vr::hud::postfx_cine_size() ? "render-target (size-only in cutscenes)"
                                                    : "render-target")
                    : "size-only");
        int dumpEdge = 0, dumpCount = 0;
        d2vr::hud::get_dump_on_edge(&dumpEdge, &dumpCount);
        XRLOG("xr: cine %s mode=%s active=%d | enters %u exits %u presents %u | "
                ...
                d2vr::hud::fov_mismatch() ? 1 : 0, d2vr::hud::screen_only() ? 1 : 0,
...
void set_aim_occluder(bool on, float x0, float y0, float x1, float y1) {
    ...
    if (was != ok)
        D2VR_LOG(::d2vr::log::Cat::present, ::d2vr::log::Level::Info,
...
void set_mirror_hook(MirrorHook fn) {
    g_mirrorHook = fn;
    D2VR_INFO("xr: desktop eye pin %s", fn ? "installed - the game window will "
...
void set_mono_context(d2vr::mono::Context context,bool forceMono) {
...
void set_mono_context(d2vr::mono::Context,bool) {}
