...
#define D2VR_CAT ::d2vr::log::Cat::openxr
...
bool render_yaw_deg(float* outDeg, uint32_t* outSerial)
{
    ...
    if (g_vpConv == 0) {
        ...
        if (g_vpConv == 0) {
            g_vpConv = -1;
            D2VR_LOG(D2VR_CAT, ::d2vr::log::Level::Warn,
...
static void RunArithmeticControls()
{
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
        ...
        ok ? ++pass : ++fail;
        if (!ok)
            D2VR_LOG(D2VR_CAT, ::d2vr::log::Level::Error,
    ...
    D2VR_LOG(D2VR_CAT, fail ? ::d2vr::log::Level::Error : ::d2vr::log::Level::Info,
...
void check_controls(const Record& real, float observedYawDeg)
{
    ...
    if (which == CTRL_YAW) {
        ...
        D2VR_LOG(D2VR_CAT, pass ? ::d2vr::log::Level::Info : ::d2vr::log::Level::Error,
    ...
    if (which == CTRL_WRONG_EYE) {
        ...
        pass ? ++g_ctrlPass : ++g_ctrlFail;
        D2VR_LOG(D2VR_CAT, pass ? ::d2vr::log::Level::Info : ::d2vr::log::Level::Warn,
                    ...
                    uint32_t submittedGen, int lagUsed, const Record& rec)
{
    ...
    D2VR_LOG_EVERY_MS(D2VR_CAT, ::d2vr::log::Level::Info, 1000,
...
void log_beat()
{
    ...
    if (!have) {
        ...
        D2VR_LOG_EVERY_MS(D2VR_CAT, ::d2vr::log::Level::Info, 3000,
    ...
    D2VR_LOG_EVERY_MS(D2VR_CAT, ::d2vr::log::Level::Info, 3000,
        ...
        d2vr::clock::now_ms() - last.cam.writeMs,
