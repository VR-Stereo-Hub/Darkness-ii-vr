// d2vr_xrsim32 logging and data directory.
//
...
void init() {
    ...
    // D2VR_XRSIM_DIR exists so a self-test process and a live game session cannot
    ...
    DWORD n = GetEnvironmentVariableW(L"D2VR_XRSIM_DIR", g_dir, MAX_PATH);
    if (n == 0 || n >= MAX_PATH) {
        ...
        if (slash) {
            *slash = 0;
            ...
            swprintf_s(g_dir, L"%s\\Darkness2VR", local);
