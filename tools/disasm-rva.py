...
#
# Ported from the Dishonored VR mod (method only). Every RVA the mod relies on
# is derived with this tool and recorded in docs/darkness2/ENGINE_NOTES.md with
# its derivation; the numbers live in src/game/darkness2/patterns.h.
#
...
# ENGINE_NOTES instead. The TOOL is ours and is committed; its OUTPUT is the
# game's and is not.
#
# Requires capstone (`pip install capstone`; verified with Python 3.14 on 2026-09-25).
#
...
#
# `disp` disassembles a range and reports every instruction whose memory operand
# uses a given structure displacement (the way to find who touches one field of
# an engine object). `float` + `xref` is the complementary route - locate the
# constant in .rdata, then find the code that loads it. `calls` on a function
# the engine must call every frame returning ZERO means the dispatch is virtual
# (this engine's message pump is one such: see ENGINE_NOTES).
#
# DarknessII.exe has no ASLR (ImageBase 0x400000), so VA = 0x400000 + RVA; every
# command here takes RVAs and prints both.
...
class Pe:
    ...
    def __init__(self, path):
        ...
        if machine != 0x14C:
            raise ValueError("not 32-bit x86 (machine 0x%X) - DarknessII.exe is Win32" % machine)
...
def cmd_dis(pe, a):
    ...
    for ins in m.disasm(blob, pe.image_base + start):
        ...
        mark = "  <== " if r == rva else "      "
        print("%s%08X va %08X  %-24s %s %s" %
              (mark, r, ins.address, ins.bytes.hex().upper(), ins.mnemonic, ins.op_str))
        shown += 1
