...
# installing the hook, or that its dispatch is virtual (a vtable slot: many
# .rdata dword references, few or no E8 callers). Ported from the Dishonored VR
# mod, method only; it embeds nothing of either game.
#
...
#
# Examples:
#   .\tools\pe-xref.ps1 -Exe "D:\...\Darkness II\DarknessII.exe" -TargetRva 8F2C30,72E5E0
#   .\tools\pe-xref.ps1 -Exe $exe -TargetRva 72E5E0 -FollowStubs -Mode Both
param(
...
# because caller lists are game-derived content (docs/darkness2/ENGINE_NOTES.md
# gets the summary, never the list).
if ($OutFile) {
...
if ($optMagic -ne 0x10B) { throw ("Only PE32 is supported (magic 0x{0:X}). " +
                                  "DarknessII.exe is 32-bit." -f $optMagic) }
