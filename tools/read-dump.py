...
uses at dump time - no dbghelp, no symbols). Ported from the Dishonored VR mod.
...
findings in docs/darkness2/ENGINE_NOTES.md, never commit the raw output.
...
def main():
    ...
    for base, end, name in mods:
        n = name.lower()
        if any(k in n for k in ("darkness", "d3d9", "d2vr", "xinput", "openxr", "d3d11",
                                "dxgi", "user32", "kernel32", "ntdll", "steam", "overlay")):
