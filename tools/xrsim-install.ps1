...
param(
    [switch]$Release,
    [string]$Dir = "$env:LOCALAPPDATA\Darkness2VR\xrsim",
...
$dll    = Join-Path $outDir "d2vr_xrsim32.dll"

if (-not (Test-Path $dll)) {
    throw "d2vr_xrsim32.dll not found in $outDir - run .\tools\build.ps1$(if ($Release) {' -Release'}) first."
}
...
$escaped  = $dll.Replace('\', '\\')
$json     = '{"file_format_version":"1.0.0","runtime":{"name":"d2vr-xrsim","library_path":"' + $escaped + '"}}'
