# xrsim-launch.ps1 - launch The Darkness II against the SIMULATED OpenXR runtime.
#
...
# the static OpenXR loader inside d3d9.dll) and D2VR_XRSIM_DIR (where the sim
...
# D2VR_XRSIM_DIR cannot reach the game either; the simulator then takes its state
...
# D2VR_DATA_DIR when the default location is not shared for real - gotcha 14).
#
...
$exe = Join-Path $GamePath "DarknessII.exe"
$modLog = Join-Path $GamePath "darkness2_vr.log"
...
$manifest = Join-Path $Dir "d2vr_xrsim32.json"
...
# --- launch ------------------------------------------------------------------
$iniPath = Join-Path $GamePath "darkness2_vr.ini"
$iniBackup = $null
...
$savedDir     = $env:D2VR_XRSIM_DIR
$p = $null
try {
    $env:XR_RUNTIME_JSON = $manifest
    $env:D2VR_XRSIM_DIR   = $Dir
    if ($ViaSteam) {
        ...
        Write-Host "launching DarknessII.exe directly with XR_RUNTIME_JSON -> the simulator"
...
# The mod logs: xr: runtime "d2vr-xrsim"   then   xr: pipeline READY
$runtimeName = $null
$sessionUp   = $false
for ($i = 0; $i -lt $WaitSeconds; $i++) {
    if (Test-Path $modLog) {
        ...
        if ($runtimeName -and $runtimeName -ne "d2vr-xrsim") {
            Restore-Ini
