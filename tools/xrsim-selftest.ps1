...
param(
    [switch]$Release,
    [string]$Dir = "$env:LOCALAPPDATA\Darkness2VR\xrsim\selftest",
    [switch]$KeepFiles,
...
try {
    ...
    $env:D2VR_XRSIM_DIR   = $savedD
}

Write-Host $out

# Catches "the loader picked the real runtime anyway" before anything else.
if ($out -notmatch "runtime:\s*d2vr-xrsim") {
    Fail "xr_hello32 did not report runtime 'd2vr-xrsim'. XR_RUNTIME_JSON did not take (elevated shell? bad manifest path? 64-bit dll?)."
}
...
Write-Host ""
Write-Host "SELFTEST PASS: d2vr-xrsim ran $($s.frame) frames, reached $($s.sessionState), 0 errors." -ForegroundColor Green
exit 0
